...
- [ ] Cupons, cashback, banners, blog: ainda não têm admin (schema já existe para cupons/cashback/banners/blog)
...
1. Perguntar ao usuário o escopo real do "editor visual estilo Wix" (ver seção acima) — provavelmente começar por banners/textos editáveis da Home.
2. Perguntas de produto (schema já existe: `Question`, só falta a UI — mesmo padrão de `Review`).
...
4. Admin de Cupons/Cashback/Banners/Blog (schema já existe, falta CRUD).
5. Lighthouse audit formal (Performance/SEO/A11y/Best Practices) quando o site tiver mais conteúdo real.
