...
### Fases 6+ (Admin, integrações reais, SEO, qualidade, produção) — NÃO INICIADAS
...
1. Busca funcional no Header (campo existe, sem submit handler).
2. Avaliações e perguntas de produto (schema já existe: `Review`, `Question`).
3. Painel Admin (Fase 6) — CRUD de produtos/pedidos, RBAC já modelado via `UserRole`.
4. Trocar frete/pagamento simulados pelas integrações reais assim que houver credenciais de sandbox.
